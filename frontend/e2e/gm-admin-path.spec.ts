...
test.describe('GM/Admin Path', () => {
  test('Setup Wizard -> Role Audit -> LSW -> Obeya SQDCP -> System Health', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /system administration/i })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByRole('heading', { level: 1, name: /cognitive obeya/i })).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByRole('heading', { level: 1, name: /ai-powered analytics/i })).toBeVisible({ timeout: 10000 });
    await expect(page.locator('text=Model Health Status')).toBeVisible();
