...
test.describe('CEO/Exec Path', () => {
  test('North Star -> NL2SQL -> Employee Risk -> Export', async ({ page }) => {
    ...
    // Ensure we are on executive route after auth store hydration.
    await page.goto('/executive');
    ...
    const nl2sqlResult = page.locator('[data-testid="nl2sql-result"]');
    const nl2sqlError = page.locator('[data-testid="nl2sql-error"]');
    await expect(nl2sqlResult.or(nl2sqlError)).toBeVisible({ timeout: 20000 });
    if (await nl2sqlResult.isVisible()) {
      await expect(nl2sqlResult).toContainText(/open_capas/i);
    } else {
      await expect(nl2sqlError).toBeVisible();
    }
    ...
    await expect(page.locator('[data-testid="risk-result"]').or(page.locator('[data-testid="risk-error"]'))).toBeVisible({ timeout: 20000 });

    // 4) Strategic export entry point is available
    const exportLink = page.locator('a[href*="/api/v1/executive/strategic-report/export"]').first();
