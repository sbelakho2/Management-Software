...
pub async fn sales_flow_impact(
    ...
    let available_hours: Decimal = sqlx::query_scalar(
        "SELECT COALESCE(SUM(available_hours_per_day), 0)::numeric FROM work_centers \
    ...
    // The stream's current best takt (from effective standards).
    let current_takt: Option<Decimal> = sqlx::query_scalar(
        "SELECT MIN(takt_time_seconds)::numeric FROM standard_work_documents \
    ...
    // CTQs of this product family imply inspection capability.
    if let Ok(ctqs) = sqlx::query_scalar::<_, String>(
        "SELECT name FROM ctq_characteristics WHERE tenant_id = $1 AND is_active = TRUE LIMIT 3",
    )
    .bind(user.tenant_id)
    ...
    let supplier_lead: i64 = sqlx::query_scalar(
        "SELECT COALESCE(MAX(lead_time_days), 0) FROM products \
         WHERE tenant_id = $1 AND primary_supplier_id IS NOT NULL",
    )
    .bind(user.tenant_id)
    .fetch_one(pool.as_ref())
    .await
    ...
    let assembly_lead: i64 = sqlx::query_scalar(
        "SELECT COALESCE(SUM(lead_time_days), 0) FROM products WHERE tenant_id = $1",
    )
    .bind(user.tenant_id)
    .fetch_one(pool.as_ref())
    .await
