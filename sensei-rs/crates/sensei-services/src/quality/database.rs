...
impl QualityService for DatabaseQualityService {
    ...
    async fn get_ncr(&self, tenant_id: Uuid, id: Uuid) -> Result<NonConformance> {
        ...
        serde_json::from_value(row.data).map_err(|e| SenseiError::Database(format!("Failed to deserialize NCR: {e}")))
    }

    async fn update_ncr_status(&self, tenant_id: Uuid, id: Uuid, severity: NcSeverity) -> Result<NonConformance> {
        let mut ncr = self.get_ncr(tenant_id, id).await?;
        ncr.severity = severity; ncr.updated_at = Utc::now();
        let data = serde_json::to_value(&ncr).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_ncrs SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(ncr.updated_at).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_ncr_status", e))?;
        Ok(ncr)
    }

    // ── CAPAs ─────────────────────────────────────────────────────────────

    async fn list_capas(&self, tenant_id: Uuid, status: Option<&str>, nc_type: Option<&str>, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<CapaExtended>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
    ...
    async fn create_capa(&self, tenant_id: Uuid, title: String, description: String, nc_ids: Vec<Uuid>, capa_type: CapaType, priority: CapaPriority, owner_id: Option<Uuid>, due_date: Option<chrono::DateTime<Utc>>) -> Result<CapaExtended> {
        let now = Utc::now(); let (id, capa_number) = gen_number("CAPA");
        let capa = CapaExtended { id, capa_number, title, description, nc_ids, capa_type, priority, status: CapaStatusEx::Draft, root_cause_analyses: vec![], actions: vec![], closure_gates: vec![], effectiveness_checks: vec![], entity_links: vec![], owner_id, due_date, closed_at: None, created_at: now, updated_at: now };
    ...
    async fn get_capa(&self, tenant_id: Uuid, id: Uuid) -> Result<CapaExtended> {
        ...
        serde_json::from_value(row.data).map_err(|e| SenseiError::Database(format!("Failed to deserialize CAPA: {e}")))
    }

    async fn update_capa_status(&self, tenant_id: Uuid, id: Uuid, status: CapaStatusEx) -> Result<CapaExtended> {
        let mut capa = self.get_capa(tenant_id, id).await?;
        capa.status = status;
        if matches!(capa.status, CapaStatusEx::Closed) { capa.closed_at = Some(Utc::now()); }
        capa.updated_at = Utc::now();
        let data = serde_json::to_value(&capa).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_capas SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(capa.updated_at).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_capa_status", e))?;
        Ok(capa)
    }

    // ── Inspections ───────────────────────────────────────────────────────

    async fn list_first_article_inspections(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<FirstArticleInspection>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_first_article_inspections WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_fai", e))?;
        let items: Vec<FirstArticleInspection> = rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect();
        Ok(paginate(items, count, page, pp))
    }

    async fn create_first_article_inspection(&self, tenant_id: Uuid, mut fai: FirstArticleInspection) -> Result<FirstArticleInspection> {
        let now = Utc::now(); fai.id = Uuid::new_v4(); fai.created_at = now; fai.updated_at = now;
    ...
    async fn list_self_inspections(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<SelfInspection>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_self_inspections WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_si", e))?;
        let items: Vec<SelfInspection> = rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect();
        Ok(paginate(items, count, page, pp))
    }

    async fn create_self_inspection(&self, tenant_id: Uuid, mut inspection: SelfInspection) -> Result<SelfInspection> {
        let now = Utc::now(); inspection.id = Uuid::new_v4(); inspection.created_at = now;
    ...
    async fn list_audits(&self, tenant_id: Uuid, status: Option<&str>, audit_type: Option<&str>, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<Audit>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
    ...
    async fn create_audit(&self, tenant_id: Uuid, mut audit: Audit) -> Result<Audit> {
        let now = Utc::now(); audit.id = Uuid::new_v4(); audit.created_at = now; audit.updated_at = now;
    ...
    async fn get_audit(&self, tenant_id: Uuid, id: Uuid) -> Result<Audit> {
        ...
        serde_json::from_value(row.data).map_err(|e| SenseiError::Database(format!("Failed to deserialize audit: {e}")))
    }

    async fn list_audit_findings(&self, tenant_id: Uuid, audit_id: Uuid) -> Result<Vec<AuditFinding>> {
        ...
        Ok(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect())
    }

    // ── Supplier Quality ──────────────────────────────────────────────────

    async fn list_supplier_scorecards(&self, tenant_id: Uuid, supplier_id: Option<Uuid>, period: Option<&str>, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<SupplierScorecard>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_supplier_scorecards WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_scorecards", e))?;
        let items: Vec<SupplierScorecard> = rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok())
            .filter(|sc: &SupplierScorecard| supplier_id.is_none_or(|sid| sc.supplier_id == sid.to_string()) && period.is_none_or(|p| sc.period_key == p)).collect();
        Ok(paginate(items, count, page, pp))
    }

    async fn create_supplier_evaluation(&self, tenant_id: Uuid, mut scorecard: SupplierScorecard) -> Result<SupplierScorecard> {
    ...
    async fn list_scars(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<Scar>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_scars WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_scars", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    // ── Documents ─────────────────────────────────────────────────────────

    async fn list_documents(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<QmsDocument>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_documents WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_docs", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    // ── MSA / SPC / Process Capability ────────────────────────────────────

    async fn list_msa_studies(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<MsaStudy>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_msa_studies WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_msa", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    async fn list_process_capability_studies(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<ProcessCapabilityStudy>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_process_capability_studies WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_pc", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    async fn list_control_plans(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<ControlPlan>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_control_plans WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_cp", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    async fn list_pfmeas(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<PfmeaLite>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_pfmeas WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_pfmeas", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    // ── NPI ───────────────────────────────────────────────────────────────

    async fn list_npi_projects(&self, tenant_id: Uuid, stage: Option<&str>, status: Option<&str>, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<NpiProject>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
    ...
    async fn list_npi_risks(&self, tenant_id: Uuid, project_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<NpiRisk>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_npi_risks WHERE tenant_id=$1 AND project_id=$2").bind(tenant_id).bind(project_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_npi_risks", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    // ── Gauges ────────────────────────────────────────────────────────────

    async fn list_gauges(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<Gauge>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_gauges WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_gauges", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    // ── Complaints / 8D / Reviews ─────────────────────────────────────────

    async fn list_complaints(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<CustomerComplaint>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_complaints WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_complaints", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    async fn list_eight_d_reports(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<EightDReport>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_eight_d_reports WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_8d", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }

    async fn list_management_reviews(&self, tenant_id: Uuid, page: Option<usize>, per_page: Option<usize>) -> Result<PaginatedResponse<ManagementReview>> {
        let page = page.unwrap_or(1).max(1); let pp = per_page.unwrap_or(20).clamp(1, 100); let off = (page - 1) * pp;
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM quality_management_reviews WHERE tenant_id=$1").bind(tenant_id).fetch_one(&self.pool).await.map_err(|e| db_err("count_reviews", e))?;
        Ok(paginate(rows.into_iter().filter_map(|r| serde_json::from_value(r.data).ok()).collect(), count, page, pp))
    }
    ...
    async fn update_ncr(&self, tenant_id: Uuid, id: Uuid, ncr: NonConformance) -> Result<NonConformance> {
        ...
        let data = serde_json::to_value(&ncr).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_ncrs SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_ncr", e))?;
        Ok(ncr)
    }

    async fn delete_ncr(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_ncrs WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_ncr", e))?;
        if r.rows_affected() == 0 { return Err(not_found("NCR", id)); }
        Ok(())
    }
    ...
    async fn update_capa(&self, tenant_id: Uuid, id: Uuid, capa: CapaExtended) -> Result<CapaExtended> {
        ...
        let data = serde_json::to_value(&capa).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_capas SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_capa", e))?;
        Ok(capa)
    }

    async fn delete_capa(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_capas WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_capa", e))?;
        if r.rows_affected() == 0 { return Err(not_found("CAPA", id)); }
        Ok(())
    }
    ...
    async fn close_capa(&self, tenant_id: Uuid, id: Uuid) -> Result<CapaExtended> {
        self.update_capa_status(tenant_id, id, CapaStatusEx::Closed).await
    }
    ...
    async fn delete_first_article_inspection(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_first_article_inspections WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_fai", e))?;
        if r.rows_affected() == 0 { return Err(not_found("FAI", id)); }
        Ok(())
    }
    ...
    async fn delete_self_inspection(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_self_inspections WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_si", e))?;
        if r.rows_affected() == 0 { return Err(not_found("SelfInspection", id)); }
        Ok(())
    }
    ...
    async fn update_audit(&self, tenant_id: Uuid, id: Uuid, audit: Audit) -> Result<Audit> {
        let now = Utc::now();
        let data = serde_json::to_value(&audit).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_audits SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_audit", e))?;
        Ok(audit)
    }

    async fn delete_audit(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_audits WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_audit", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Audit", id)); }
        Ok(())
    }
    ...
    async fn update_supplier_scorecard(&self, tenant_id: Uuid, id: Uuid, scorecard: SupplierScorecard) -> Result<SupplierScorecard> {
        ...
        sqlx::query("UPDATE quality_supplier_scorecards SET data=$1 WHERE id=$2 AND tenant_id=$3")
            .bind(&data).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_scorecard", e))?;
        Ok(scorecard)
    }

    async fn delete_supplier_scorecard(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_supplier_scorecards WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_scorecard", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Scorecard", id)); }
        Ok(())
    }

    async fn create_scar(&self, tenant_id: Uuid, mut scar: Scar) -> Result<Scar> {
        let now = Utc::now(); let (id, scar_number) = gen_number("SCAR");
        scar.id = id; scar.scar_number = scar_number; scar.created_at = now; scar.updated_at = now;
    ...
    async fn update_scar(&self, tenant_id: Uuid, id: Uuid, scar: Scar) -> Result<Scar> {
        ...
        sqlx::query("UPDATE quality_scars SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_scar", e))?;
        Ok(scar)
    }

    async fn delete_scar(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_scars WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_scar", e))?;
        if r.rows_affected() == 0 { return Err(not_found("SCAR", id)); }
        Ok(())
    }
    ...
    async fn update_document(&self, tenant_id: Uuid, id: Uuid, doc: QmsDocument) -> Result<QmsDocument> {
        let now = Utc::now();
        let data = serde_json::to_value(&doc).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_documents SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_doc", e))?;
        Ok(doc)
    }

    async fn delete_document(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_documents WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_doc", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Document", id)); }
        Ok(())
    }
    ...
    async fn create_msa_study(&self, tenant_id: Uuid, study: MsaStudy) -> Result<MsaStudy> {
        let now = Utc::now(); let id = study.id;
    ...
    async fn delete_msa_study(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_msa_studies WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_msa", e))?;
        if r.rows_affected() == 0 { return Err(not_found("MSA Study", id)); }
        Ok(())
    }
    ...
    async fn delete_process_capability_study(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_process_capability_studies WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_pc", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Process Capability Study", id)); }
        Ok(())
    }
    ...
    async fn update_control_plan(&self, tenant_id: Uuid, id: Uuid, cp: ControlPlan) -> Result<ControlPlan> {
        ...
        sqlx::query("UPDATE quality_control_plans SET data=$1 WHERE id=$2 AND tenant_id=$3")
            .bind(&data).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_cp", e))?;
        Ok(cp)
    }

    async fn delete_control_plan(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_control_plans WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_cp", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Control Plan", id)); }
        Ok(())
    }
    ...
    async fn create_pfmea(&self, tenant_id: Uuid, pfmea: PfmeaLite) -> Result<PfmeaLite> {
        let now = Utc::now(); let id = pfmea.id;
        let data = serde_json::to_value(&pfmea).unwrap_or(serde_json::Value::Null);
        sqlx::query("INSERT INTO quality_pfmeas (id, tenant_id, data, created_at) VALUES ($1,$2,$3,$4)")
            .bind(id).bind(tenant_id).bind(&data).bind(now).execute(&self.pool).await.map_err(|e| db_err("create_pfmea", e))?;
        Ok(pfmea)
    }

    async fn delete_pfmea(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_pfmeas WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_pfmea", e))?;
        if r.rows_affected() == 0 { return Err(not_found("PFMEA", id)); }
        Ok(())
    }
    ...
    async fn create_npi_project(&self, tenant_id: Uuid, project: NpiProject) -> Result<NpiProject> {
        let now = Utc::now(); let id = project.id;
    ...
    async fn update_npi_project(&self, tenant_id: Uuid, id: Uuid, project: NpiProject) -> Result<NpiProject> {
        let now = Utc::now();
        let data = serde_json::to_value(&project).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_npi_projects SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_npi", e))?;
        Ok(project)
    }

    async fn delete_npi_project(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_npi_projects WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_npi", e))?;
        if r.rows_affected() == 0 { return Err(not_found("NPI Project", id)); }
        Ok(())
    }
    ...
    async fn create_gauge(&self, tenant_id: Uuid, gauge: Gauge) -> Result<Gauge> {
        let now = Utc::now(); let id = gauge.id;
        let data = serde_json::to_value(&gauge).unwrap_or(serde_json::Value::Null);
        sqlx::query("INSERT INTO quality_gauges (id, tenant_id, data, created_at) VALUES ($1,$2,$3,$4)")
            .bind(id).bind(tenant_id).bind(&data).bind(now).execute(&self.pool).await.map_err(|e| db_err("create_gauge", e))?;
        Ok(gauge)
    }
    ...
    async fn update_gauge(&self, tenant_id: Uuid, id: Uuid, gauge: Gauge) -> Result<Gauge> {
        ...
        sqlx::query("UPDATE quality_gauges SET data=$1 WHERE id=$2 AND tenant_id=$3")
            .bind(&data).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_gauge", e))?;
        Ok(gauge)
    }

    async fn delete_gauge(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_gauges WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_gauge", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Gauge", id)); }
        Ok(())
    }
    ...
    async fn create_complaint(&self, tenant_id: Uuid, mut complaint: CustomerComplaint) -> Result<CustomerComplaint> {
        let now = Utc::now(); let (id, complaint_number) = gen_number("CMP");
        complaint.id = id; complaint.complaint_number = complaint_number; complaint.created_at = now; complaint.updated_at = now;
    ...
    async fn update_complaint(&self, tenant_id: Uuid, id: Uuid, complaint: CustomerComplaint) -> Result<CustomerComplaint> {
        let now = Utc::now();
        let data = serde_json::to_value(&complaint).unwrap_or(serde_json::Value::Null);
        sqlx::query("UPDATE quality_complaints SET data=$1, updated_at=$2 WHERE id=$3 AND tenant_id=$4")
            .bind(&data).bind(now).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_complaint", e))?;
        Ok(complaint)
    }

    async fn delete_complaint(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_complaints WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_complaint", e))?;
        if r.rows_affected() == 0 { return Err(not_found("Complaint", id)); }
        Ok(())
    }
    ...
    async fn create_eight_d_report(&self, tenant_id: Uuid, report: EightDReport) -> Result<EightDReport> {
        let now = Utc::now(); let id = report.id;
    ...
    async fn update_eight_d_report(&self, tenant_id: Uuid, id: Uuid, report: EightDReport) -> Result<EightDReport> {
        ...
        sqlx::query("UPDATE quality_eight_d_reports SET data=$1 WHERE id=$2 AND tenant_id=$3")
            .bind(&data).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_8d", e))?;
        Ok(report)
    }

    async fn delete_eight_d_report(&self, tenant_id: Uuid, id: Uuid) -> Result<()> {
        let r = sqlx::query("DELETE FROM quality_eight_d_reports WHERE id=$1 AND tenant_id=$2").bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("delete_8d", e))?;
        if r.rows_affected() == 0 { return Err(not_found("8D Report", id)); }
        Ok(())
    }
    ...
    async fn create_management_review(&self, tenant_id: Uuid, review: ManagementReview) -> Result<ManagementReview> {
        let now = Utc::now(); let id = review.id;
    ...
    async fn update_management_review(&self, tenant_id: Uuid, id: Uuid, review: ManagementReview) -> Result<ManagementReview> {
        ...
        sqlx::query("UPDATE quality_management_reviews SET data=$1 WHERE id=$2 AND tenant_id=$3")
            .bind(&data).bind(id).bind(tenant_id).execute(&self.pool).await.map_err(|e| db_err("update_review", e))?;
        Ok(review)
    }
