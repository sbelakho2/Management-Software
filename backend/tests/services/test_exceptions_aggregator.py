...
@pytest.fixture
def db() -> AsyncSession:
    """Create a mock database session with basic persistence simulation."""
    session = AsyncMock(spec=AsyncSession)
    persisted_records = []

    def mock_add(instance):
        from sensei.models.exception import ExceptionRecord
        if isinstance(instance, ExceptionRecord):
            persisted_records.append(instance)
    ...
    async def mock_execute(statement, *args, **kwargs):
        ...
        # Handle SELECT via SQLAlchemy select() construct
        from sqlalchemy.sql.selectable import Select
        if isinstance(statement, Select):
            mock_result.scalars.return_value.all.return_value = persisted_records
            return mock_result
        
        # Handle UPDATE via SQLAlchemy update() construct
        from sqlalchemy.sql.dml import Update
        if isinstance(statement, Update):
            try:
                # Try to find the ID and values to update
                # Compile statement to get params
                compiled = statement.compile()
                params = compiled.params
                
                # Try to find target ID in params (id_1, etc)
                target_id = None
                for k, v in params.items():
                    if k.startswith('id_') and isinstance(v, str):
                        target_id = v
                        break
                ...
                if target_id:
                    for rec in persisted_records:
                        if rec.id == target_id:
                            for k, v in params.items():
                                if hasattr(rec, k) and not k.startswith('id_'):
                                    setattr(rec, k, v)
            except Exception:
                pass
            return mock_result

        # Fallback for string-based or other queries
...
class TestAggregatorInitialization:
    ...
    @pytest.mark.asyncio
    async def test_creates_empty_aggregator(self, aggregator: ExceptionsAggregator, db: AsyncSession):
        """Test aggregator starts empty."""
        assert len(await aggregator.get_all(db)) == 0
...
class TestSourceRegistration:
    ...
    @pytest.mark.asyncio
    async def test_refresh_calls_sources(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
        """Test refresh calls registered sources."""
        source = MagicMock(return_value=[sample_exception])
        aggregator.register_source("test", source)
        
        await aggregator.refresh(db, force=True)
        
        source.assert_called_once()
        exceptions = await aggregator.get_all(db)
        assert len(exceptions) == 1
        assert exceptions[0].title == sample_exception.title

    @pytest.mark.asyncio
    async def test_refresh_handles_source_errors(self, aggregator: ExceptionsAggregator, db: AsyncSession):
...
class TestExceptionManagement:
    ...
    @pytest.mark.asyncio
    async def test_add_exception(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
        """Test adding an exception manually."""
        await aggregator.add_exception(db, sample_exception)
    ...
    async def test_update_exception(
        ...
        updated = await aggregator.update_exception(
            db,
            sample_exception.id,
    ...
    @pytest.mark.asyncio
    async def test_update_nonexistent_exception(self, aggregator: ExceptionsAggregator, db: AsyncSession):
        """Test updating a non-existent exception returns None."""
        result = await aggregator.update_exception(db, "nonexistent", {"status": ExceptionStatus.RESOLVED})
    ...
    async def test_resolve_exception(
        ...
        resolved = await aggregator.resolve_exception(db, sample_exception.id)
    ...
    async def test_escalate_exception(
        ...
        escalated = await aggregator.escalate_exception(
            db,
            sample_exception.id,
    ...
    @pytest.mark.asyncio
    async def test_acknowledge_exception(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
        """Test acknowledging an exception."""
        await aggregator.add_exception(db, sample_exception)
        
        acked = await aggregator.acknowledge_exception(db, sample_exception.id)
...
class TestFiltering:
    ...
    async def test_get_by_category(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        """Test filtering by category."""
        for e in sample_exceptions:
            await aggregator.add_exception(db, e)
        
        andon = await aggregator.get_by_category(db, ExceptionCategory.ANDON)
        assert len(andon) == 1
    ...
    async def test_get_by_severity(
        ...
        critical = await aggregator.get_all(db, severity=ExceptionSeverity.CRITICAL)
    ...
    async def test_get_by_status(
        ...
        open_items = await aggregator.get_all(db, status=ExceptionStatus.OPEN)
    ...
    async def test_get_overdue(
        ...
        overdue = await aggregator.get_overdue(db)
        assert len(overdue) == 1
        assert overdue[0].is_overdue is True

    @pytest.mark.asyncio
    async def test_get_critical(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        critical = await aggregator.get_critical(db)
        assert len(critical) == 2  # 1 critical + 1 high

    @pytest.mark.asyncio
    async def test_get_escalated(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        # Escalate one
        await aggregator.escalate_exception(db, sample_exceptions[0].id, "manager@example.com")
        
        escalated = await aggregator.get_escalated(db)
    ...
    async def test_get_with_limit(
        ...
        limited = await aggregator.get_all(db, limit=2)
...
class TestSummary:
    ...
    @pytest.mark.asyncio
    async def test_summary_excludes_resolved(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        # Resolve one
        await aggregator.resolve_exception(db, sample_exceptions[0].id)
        
        summary = await aggregator.get_summary(db)
        assert summary.total_open == 3

    @pytest.mark.asyncio
    async def test_summary_counts_escalated(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        await aggregator.escalate_exception(db, sample_exceptions[0].id, "manager")
        
        summary = await aggregator.get_summary(db)
        assert summary.escalated_count == 1

    @pytest.mark.asyncio
    async def test_summary_by_category(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
...
class TestNavigationBadges:
    ...
    @pytest.mark.asyncio
    async def test_get_navigation_badges(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
    ...
    @pytest.mark.asyncio
    async def test_badges_map_to_modules(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
    ...
    @pytest.mark.asyncio
    async def test_badges_use_highest_severity(self, aggregator: ExceptionsAggregator, db: AsyncSession):
        """Test badges use highest severity for module."""
        # Add two production issues with different severities
        await aggregator.add_exception(db, create_exception(
    ...
    @pytest.mark.asyncio
    async def test_badges_exclude_resolved(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        # Resolve all
        for e in sample_exceptions:
            await aggregator.resolve_exception(db, e.id)
...
class TestTrends:
    ...
    async def test_get_trends(
        ...
        trends = await aggregator.get_trends(db, days=7)
        
        assert len(trends) == 7
        assert all(hasattr(t, "critical") for t in trends)

    @pytest.mark.asyncio
    async def test_trends_count_by_severity(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
        ...
        trends = await aggregator.get_trends(db, days=1)
        today = trends[-1]
...
class TestListeners:
    ...
    @pytest.mark.asyncio
    async def test_listeners_called_on_add(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
    ...
    @pytest.mark.asyncio
    async def test_listeners_called_on_update(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
        ...
        await aggregator.update_exception(db, sample_exception.id, {"status": ExceptionStatus.ACKNOWLEDGED})
        
        listener.assert_called_once()

    @pytest.mark.asyncio
    async def test_listener_errors_dont_break_flow(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exception: ExceptionItem
    ):
...
class TestCaching:
    ...
    @pytest.mark.asyncio
    async def test_refresh_respects_cache(self, aggregator: ExceptionsAggregator, db: AsyncSession):
    ...
    @pytest.mark.asyncio
    async def test_force_refresh_ignores_cache(self, aggregator: ExceptionsAggregator, db: AsyncSession):
...
class TestSorting:
    ...
    @pytest.mark.asyncio
    async def test_exceptions_sorted_by_priority(
        self, aggregator: ExceptionsAggregator, db: AsyncSession, sample_exceptions: list[ExceptionItem]
    ):
    ...
    @pytest.mark.asyncio
    async def test_critical_appears_first(self, aggregator: ExceptionsAggregator, db: AsyncSession):
