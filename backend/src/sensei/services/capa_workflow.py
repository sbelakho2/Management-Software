...
class CAPAWorkflowIntegrationService:
    ...
    def _generate_capa_number(self) -> str:
        ...
        number = f"CAPA-{datetime.utcnow().year}-{self._next_capa_number:04d}"
        self._next_capa_number += 1
        return number
    
    def _generate_nc_number(self) -> str:
        """Generate a unique NC number."""
        number = f"NC-{datetime.utcnow().year}-{self._next_nc_number:04d}"
        self._next_nc_number += 1
        return number
    ...
    def _check_recurrence(
        ...
        cutoff_date = datetime.utcnow() - timedelta(days=self.config.recurrence_period_days)
    ...
    def verify_root_cause(
        ...
        capa.root_cause_analysis.verified_at = datetime.utcnow()
