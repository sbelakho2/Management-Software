...
class WinLossPredictionModel:
    ...
    def train(self, historical_data: list[HistoricalRFQ], epochs: int = 10) -> None:
        """Train model on historical data."""
        if len(historical_data) < MIN_SAMPLES_FOR_PREDICTION:
            return
...
class PredictiveWinLossEngine:
    ...
    def add_historical_data(self, data: list[HistoricalRFQ]) -> None:
