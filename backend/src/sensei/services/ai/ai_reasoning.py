...
.. warning:: Simulated Explanations

   The SHAP/LIME-style feature importance values generated by
   ``generate_prediction()`` and ``explain_prediction()`` are **simulated**
   using MD5 hashes of feature names — they are NOT real model explanations.
   Replace with actual SHAP/LIME integration when a trained model is available.
   See checklist items **#203, #456**.
"""
...
class AIReasoningService(PersistentServiceMixin):
    """E2E validation service for AI 2.0 capabilities."""

    SERVICE_NAME = "ai_reasoning"
    ...
    def verify_correction_applied(
        ...
        *,
        ...
        # Use n-gram overlap for more robust verification.
        # Bag-of-words alone can match semantically wrong outputs if they happen
        # to share common words.  Adding bigram overlap reduces false positives.
        corrected_lower = correction.corrected_output.lower()
        new_lower = new_output.lower()
        corrected_words = corrected_lower.split()
        new_words = new_lower.split()

        if not corrected_words:
            return False, "Correction has empty output"

        # Unigram overlap
        corrected_unigrams = set(corrected_words)
        new_unigrams = set(new_words)
        unigram_overlap = (
            len(corrected_unigrams & new_unigrams) / len(corrected_unigrams)
            if corrected_unigrams else 0.0
        )

        # Bigram overlap for structural similarity
        def _bigrams(words: list[str]) -> set[tuple[str, str]]:
            return {(words[i], words[i + 1]) for i in range(len(words) - 1)} if len(words) > 1 else set()

        corrected_bigrams = _bigrams(corrected_words)
        new_bigrams = _bigrams(new_words)
        bigram_overlap = (
            len(corrected_bigrams & new_bigrams) / len(corrected_bigrams)
            if corrected_bigrams else 0.0
        )

        # Weighted score: 60% unigram + 40% bigram (bigrams catch ordering)
        combined = 0.6 * unigram_overlap + 0.4 * bigram_overlap

        if combined > 0.6:
            correction.applied_to_next = True
            return True, f"Correction successfully applied (confidence: {combined:.0%})"

        return False, f"Correction not reflected in output (similarity: {combined:.0%})"
    ...
    def generate_prediction(
        ...
        *,
        ...
        # Use real SHAP/LIME when available (#456), otherwise use
        # proportional feature-contribution approximation.
        try:
            from sensei.services.ai.real_ml_implementations import RealExplainer
            explainer = RealExplainer()
            shap_values = explainer.explain_shap(input_features, confidence)
            lime_values = explainer.explain_lime(input_features, confidence)
        except ImportError:
            # Fallback: proportional contribution (NOT hash-based)
            total = sum(abs(v) for v in input_features.values()) or 1.0
            shap_values = {
                name: (val / total) * confidence
                for name, val in input_features.items()
            }
