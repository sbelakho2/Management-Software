...
class AICTQSummarizationService:
    ...
    def _to_utc_aware(dt: datetime) -> datetime:
        ...
        Treats naive datetimes as UTC and converts aware datetimes to UTC.
        """
    ...
    def analyze_trend(
        ...
        # Sort by date
        sorted_measurements = sorted(measurements, key=lambda m: self._to_utc_aware(m.measured_at))
    ...
    def generate_summary(
        ...
        filtered_measurements = [
            ...
            if period_start <= self._to_utc_aware(m.measured_at) <= period_end
        ]
    ...
    def _calculate_period(
        ...
        if period == AnalysisPeriod.CUSTOM and custom_start and custom_end:
            return self._to_utc_aware(custom_start), self._to_utc_aware(custom_end)
...
""".strip()
    ...
    def compare_periods(
        ...
        period1_start, period1_end = self._to_utc_aware(period1[0]), self._to_utc_aware(period1[1])
        period2_start, period2_end = self._to_utc_aware(period2[0]), self._to_utc_aware(period2[1])
        period1_measurements = [
            m for m in measurements
            if period1_start <= self._to_utc_aware(m.measured_at) <= period1_end
        ]
        period2_measurements = [
            m for m in measurements
            if period2_start <= self._to_utc_aware(m.measured_at) <= period2_end
        ]
